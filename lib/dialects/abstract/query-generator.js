  ...
  */
  ...
  escapeAttributes(attributes, options, mainTableAs) {
    return attributes && attributes.map(attr => {
      let addTable = true;
      // CVE-2023-22578: forbid unescaped attributes with parentheses (SQLi vector).
      // Behavior is controlled by sequelize option `attributeBehavior`:
      //   - 'throw' (default): reject raw attributes containing parentheses.
      //   - 'escape': auto-escape attribute as identifier (Sequelize v7 behavior).
      //   - 'unsafe-legacy': allow legacy v5 behavior (risky); emits deprecation.
      // Upstream commits: 344c404 / d3f5b5a. See advisory GHSA-f598-mfpv-gmfx.
      const attributeBehavior =
        this.sequelize.options && this.sequelize.options.attributeBehavior
          ? this.sequelize.options.attributeBehavior
          : 'throw';
      ...
      if (Array.isArray(attr)) {
        ...
        if (attr[0] instanceof Utils.SequelizeMethod) {
          ...
          addTable = false;
        } else if (attributeBehavior === 'escape' || !attr[0].includes('(') && !attr[0].includes(')')) {
          // safe path: escape identifiers, or strings without parentheses
          attr[0] = this.quoteIdentifier(attr[0]);
        } else if (attributeBehavior !== 'unsafe-legacy') {
          // block raw attribute with parentheses
          throw new Error(`Attributes cannot include parentheses in Sequelize 5:
...
https://github.com/sequelize/sequelize/discussions/15694`);
      ...
      } else if (attributeBehavior === 'escape' || !attr.includes('(') && !attr.includes(')')) {
        attr = this.quoteAttribute(attr, options.model);
      } else if (attributeBehavior !== 'unsafe-legacy') {
        throw new Error(`Attributes cannot include parentheses in Sequelize 5:
In order to fix the vulnerability CVE-2023-22578, we had to remove support for treating attributes as raw SQL if they included parentheses.
Sequelize 7 escapes all attributes, even if they include parentheses.
...
https://github.com/sequelize/sequelize/discussions/15694`);
  ...
  */
  getWhereConditions(smth, tableName, factory, options, prepend) {
    ...
    // CVE-2023-22580 / CVE-2023-22579: reject unsupported where values instead of silently returning tautology.
    // Upstream commits: 5f4e3d2, adb35c4 (and related 6c71dbd, d9e0728). The previous behavior returned '1=1',
    // which allowed crafted inputs to bypass where conditions.
