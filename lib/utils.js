...
function getComplexKeys(obj) {
  const operators = getOperators(obj);
  const stringKeys = _.keys(obj);
  const allSymbols = Object.getOwnPropertySymbols(obj || {});
  // Get symbols that are not operators
  const nonOperatorSymbols = allSymbols.filter(sym => !operatorsSet.has(sym));
  return operators.concat(stringKeys).concat(nonOperatorSymbols);
}
